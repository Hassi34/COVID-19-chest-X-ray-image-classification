...
def get_DenseNet201_model(input_shape : list[int], model_path: str, logger: RootLogger) -> tf.keras.models.Model:
    model = tf.keras.applications.densenet.DenseNet201(input_shape = input_shape,
                                                       weights = "imagenet",
                                                       include_top = False)
    
    logger.info(f"base model summary: {_get_model_summary(model)}")
    model.save(model_path)
    logger.info(f"DenseNet201 base model saved at {model_path}")
    return model
